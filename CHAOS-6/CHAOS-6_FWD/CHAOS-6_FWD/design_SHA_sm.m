    ...
    if coeffs(k) ~= 0
        ...
        for l = 1:l_max
            ...
            while abs(SM_M_sorted(k,l,n)) > 1e-6 & n < N
                y(l,:) = y(l,:) + squeeze(real(SM_M_sorted(k,l,n)*exp(-j*SM_omega_sorted(k,l,n)*t))');
                n = n+1;
            end
            n = 1;
            while abs(SM_M_sorted_ind(k,l,n)) > 1e-6 & n < N
                y_ind(l,:) = y_ind(l,:) + squeeze(real(SM_M_sorted_ind(k,l,n)*exp(-j*SM_omega_sorted_ind(k,l,n)*t))');
                n = n+1;
            end
            %         for n = 1:N
            %             y(l,:) = y(l,:) + squeeze(real(SM_M_sorted(k,l,n)*exp(-j*SM_omega_sorted(k,l,n)*t))');
...
if strfind(i_e_flag, 'e') > 0
    [A_r_geo, A_theta_geo, A_phi_geo] = design_SHA(r, theta_geo, phi_geo, 1, 'ext');
    for k = 1:min(length(coeffs), 3)
        A_r_sm(:,k)     = A_r_sm(:,k)     + squeeze(q_SM(k,1,:)).*A_r_geo(:,1)     + squeeze(q_SM(k,2,:)).*A_r_geo(:,2)     + squeeze(q_SM(k,3,:)).*A_r_geo(:,3);
        A_theta_sm(:,k) = A_theta_sm(:,k) + squeeze(q_SM(k,1,:)).*A_theta_geo(:,1) + squeeze(q_SM(k,2,:)).*A_theta_geo(:,2) + squeeze(q_SM(k,3,:)).*A_theta_geo(:,3);
        A_phi_sm(:,k)   = A_phi_sm(:,k)   + squeeze(q_SM(k,1,:)).*A_phi_geo(:,1)   + squeeze(q_SM(k,2,:)).*A_phi_geo(:,2)   + squeeze(q_SM(k,3,:)).*A_phi_geo(:,3);
    end
end

% ... induced fields
if strfind(i_e_flag, 'i') > 0
    [A_r_geo, A_theta_geo, A_phi_geo] = design_SHA(r, theta_geo, phi_geo, 1, 'int');
    for k = 2:min(length(coeffs), 3) % start with k=2 since q_10_SM does not induce
        A_r_sm(:,k)     = A_r_sm(:,k)     + squeeze(g_SM(k,1,:)).*A_r_geo(:,1)     + squeeze(g_SM(k,2,:)).*A_r_geo(:,2)     + squeeze(g_SM(k,3,:)).*A_r_geo(:,3);
        A_theta_sm(:,k) = A_theta_sm(:,k) + squeeze(g_SM(k,1,:)).*A_theta_geo(:,1) + squeeze(g_SM(k,2,:)).*A_theta_geo(:,2) + squeeze(g_SM(k,3,:)).*A_theta_geo(:,3);
        A_phi_sm(:,k)   = A_phi_sm(:,k)   + squeeze(g_SM(k,1,:)).*A_phi_geo(:,1)   + squeeze(g_SM(k,2,:)).*A_phi_geo(:,2)   + squeeze(g_SM(k,3,:)).*A_phi_geo(:,3);
    end
end
...
if length(coeffs) > 3
    % external fields ...
    if strfind(i_e_flag, 'e') > 0
        [A_r_geo, A_theta_geo, A_phi_geo] = design_SHA(r, theta_geo, phi_geo, 4, 'ext');
        for k = 4:length(coeffs)
            if coeffs(k) ~= 0
                A_r_sm(:,k)     = squeeze(q_SM(k,1,:)).*A_r_geo(:,1);
                A_theta_sm(:,k) = squeeze(q_SM(k,1,:)).*A_theta_geo(:,1);
                A_phi_sm(:,k)   = squeeze(q_SM(k,1,:)).*A_phi_geo(:,1);
                for n = 1:24
    ...
    % ... induced fields
    if strfind(i_e_flag, 'i') > 0
        [A_r_geo, A_theta_geo, A_phi_geo] = design_SHA(r, theta_geo, phi_geo, 4, 'int');
        for k = 5:length(coeffs) % start with k=5 since q_20_SM does not induce
            if coeffs(k) ~= 0
                for n = 1:24
