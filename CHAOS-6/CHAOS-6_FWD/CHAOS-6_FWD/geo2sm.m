function [theta_sm, phi_sm, varargout] = geo2sm(t, theta_geo, phi_geo)
% [theta_sm, phi_sm] = geo2gsm(t, theta_geo, phi_geo);
% [theta_sm, phi_sm, R] = geo2gsm(t, theta_geo, phi_geo);
%
% Input:  geographic theta_geo,  phi_geo  (in degree)
% Output: GSM        theta_sm, phi_sm (in degree)
...
if size(t) ~= size(theta_geo) | size(t) ~= size(phi_geo);
