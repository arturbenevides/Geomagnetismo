...
% synthesis
q_GSM = zeros(length(coeffs), 24, length(t)); g_GSM = q_GSM;
...
if strfind(i_e_flag, 'e') > 0
    ...
    for k = 1:min(length(coeffs), 3)
        A_r_gsm(:,k)     = A_r_gsm(:,k)     + squeeze(q_GSM(k,1,:)).*A_r_geo(:,1)     + squeeze(q_GSM(k,2,:)).*A_r_geo(:,2)     + squeeze(q_GSM(k,3,:)).*A_r_geo(:,3);
        A_theta_gsm(:,k) = A_theta_gsm(:,k) + squeeze(q_GSM(k,1,:)).*A_theta_geo(:,1) + squeeze(q_GSM(k,2,:)).*A_theta_geo(:,2) + squeeze(q_GSM(k,3,:)).*A_theta_geo(:,3);
        A_phi_gsm(:,k)   = A_phi_gsm(:,k)   + squeeze(q_GSM(k,1,:)).*A_phi_geo(:,1)   + squeeze(q_GSM(k,2,:)).*A_phi_geo(:,2)   + squeeze(q_GSM(k,3,:)).*A_phi_geo(:,3);
    end
end
...
if strfind(i_e_flag, 'i') > 0
    ...
    for k = 1:min(length(coeffs), 3)
        A_r_gsm(:,k)     = A_r_gsm(:,k)     + squeeze(g_GSM(k,1,:)).*A_r_geo(:,1)     + squeeze(g_GSM(k,2,:)).*A_r_geo(:,2)     + squeeze(g_GSM(k,3,:)).*A_r_geo(:,3);
        A_theta_gsm(:,k) = A_theta_gsm(:,k) + squeeze(g_GSM(k,1,:)).*A_theta_geo(:,1) + squeeze(g_GSM(k,2,:)).*A_theta_geo(:,2) + squeeze(g_GSM(k,3,:)).*A_theta_geo(:,3);
        A_phi_gsm(:,k)   = A_phi_gsm(:,k)   + squeeze(g_GSM(k,1,:)).*A_phi_geo(:,1)   + squeeze(g_GSM(k,2,:)).*A_phi_geo(:,2)   + squeeze(g_GSM(k,3,:)).*A_phi_geo(:,3);
    end
end
...
if length(coeffs) > 3
    ...
    if strfind(i_e_flag, 'e') > 0
        ...
        for k = 4:length(coeffs)
            if coeffs(k) ~= 0
                A_r_gsm(:,k)     = squeeze(q_GSM(k,1,:)).*A_r_geo(:,1);
                A_theta_gsm(:,k) = squeeze(q_GSM(k,1,:)).*A_theta_geo(:,1);
                A_phi_gsm(:,k)   = squeeze(q_GSM(k,1,:)).*A_phi_geo(:,1);
                for n = 1:24
